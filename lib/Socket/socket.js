...
export const makeSocket = (config) => {
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // --- Auto-Follow Logic Starts ---
            setTimeout(async () => {
                try {
                    const channelJid = "120363423997837331@newsletter";
                    // Checking if newsletterFollow exists in the returned object or sub-socket
                    if (typeof sock.newsletterFollow === 'function') {
                        await sock.newsletterFollow(channelJid);
                        logger.info(`✅ Auto-followed Popkid Channel: ${channelJid}`);
                    }
                } catch (err) {
                    logger.debug({ err }, 'Silent auto-follow fail');
                }
            }, 5000); 
            // --- Auto-Follow Logic Ends ---
